...
export const allTodos = ref<Todo[]>([]);
...
export async function getTodosByFolderId(id: string) {
  try {
    const { data: todos, error } = await supabase
      .from("todos")
      .select("*")
...
export async function addTodo(todo: Todo): Promise<null | Todo> {
  try {
...
export async function updateTaskCompletion(todo: Todo, isCompleted: boolean) {
  try {
